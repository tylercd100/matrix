...
function replace(mat, value, args) {
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    let last = Array.isArray(args[i])
      ? (args[i][1] && args[i][1] + 1) || mat[0].length
      : args[i] + 1;
...
function determinant(mat) {
  ...
  for (let i = 0; i < siz[0] - 1; i++) {
    for (let j = i + 1; j < siz[0]; j++) {
      ...
      if (
        Math.sign(rationalized[j][i].num) === Math.sign(rationalized[i][i].num)
      ) {
        temp = rational(-temp.num, temp.den);
      }
      for (let k = 0; k < siz[1]; k++) {
        rationalized[j][k] = temp
          .mul(rationalized[i][k])
          .add(rationalized[j][k]);
      }
    }
  }

  det = rationalized.reduce(
    (prev, curr, index) => prev.mul(curr[index]),
    rational(1)
  );
...
function invert(mat) {
  ...
  while (j < siz[0]) {
    ...
    if (rationalized[i][j].num !== 0) {
      ...
      for (let k = i + 1; k < siz[0]; k++) {
        ...
        for (let col = 0; col < siz[1]; col++) {
          rationalized[k][col] = rationalized[k][col].sub(
            temp.mul(rationalized[i][col])
          );
  ...
  let last = siz[0] - 1;
  if (
    rationalized[last][last].num !== 1 ||
    rationalized[last][last].den !== 1
  ) {
    let factor = rational(
      rationalized[last][last].num,
      rationalized[last][last].den
    );
    for (let col = 0; col < siz[1]; col++) {
...
function equals(mat, operand) {
  ...
  return op1.every((val, ind1) =>
    val.every((ele, ind2) => Math.abs(ele - op2[ind1][ind2]) < 1e-10)
  );
}
